...
var xmin = move.axes[global.Machine_axis_number[0]].min                                                                                                        ; Capture X axis minimum limit for safety check.
var xmax = move.axes[global.Machine_axis_number[0]].max                                                                                                        ; Capture X axis maximum limit for safety check.
var ymin = move.axes[global.Machine_axis_number[1]].min                                                                                                        ; Capture Y axis minimum limit for safety check.
var ymax = move.axes[global.Machine_axis_number[1]].max                                                                                                        ; Capture Y axis maximum limit for safety check.
...
if global.Machine_cut_direction == "left"
    set var.location_factor = {1, 0}                                                                                                ; Initial move is away from cut, along X+.
if global.Machine_cut_direction == "right"
    set var.location_factor = {-1, 0}                                                                                               ; Initial move is away from cut, along X-.
if global.Machine_cut_direction == "front"
    set var.location_factor = {0, 1}                                                                                                ; Initial move is away from cut, along Y+.
if global.Machine_cut_direction == "back"
    set var.location_factor = {0, -1}                                                                                               ; Initial move is away from cut, along Y-.

; --- Calculate Pre-Cut Park Position (pin_park_x/y_loc) ---
if global.Machine_cut_direction == "left" || global.Machine_cut_direction == "right"
    set var.pin_park_x_loc = global.Machine_cut_location[0] + (var.location_factor[0] * global.Machine_cut_dist[0])                        ; Calculate X park position.
    set var.pin_park_y_loc = global.Machine_cut_location[1]                                                                            ; Y remains constant.
elif global.Machine_cut_direction == "front" || global.Machine_cut_direction == "back"
    set var.pin_park_x_loc = global.Machine_cut_location[0]
...
M98 P"0:/sys/AFC/Macros/debug.g" A"AFC_Cut: Lift Z"                                                                                       ; Log Z lift.
G91                                                                                                                                ; Switch to Relative Positioning mode.
G1 Z5 E-1 F{global.Machine_travel_speed[1] * 60}                                                                                         ; Lift Z by 5mm and retract E by 1mm.
G90                                                                                                                                ; Switch back to Absolute Positioning mode.

if global.Machine_cut_retract_length > 0                                                                                               ; Check if pre-cut retract is configured.
    M98 P"0:/sys/AFC/Macros/debug.g" A"Machine_Cut: Retract Filament for Cut"
    G1 E{-global.Machine_cut_retract_length} F{global.Machine_cut_move[5] * 60}                                                              ; Retract filament by configured length.
    if global.Machine_cut_quick_tip_forming                                                                                            ; Perform quick tip forming (small push/pull).
        M98 P"0:/sys/AFC/Macros/debug.g" A"Machine_Cut: Performing Quick Tip Form"
        G1 E{global.Machine_cut_retract_length/2} F{global.Machine_cut_move[5] * 60}                                                         ; Push E forward half retract length.
        G1 E{-global.Machine_cut_retract_length/2} F{global.Machine_cut_move[5] * 60}                                                        ; Pull E back half retract length.
    set var.extruder_move_dist = var.extruder_move_dist + global.Machine_cut_retract_length                                            ; Update E distance tracked.

M98 P"0:/sys/AFC/Macros/debug.g" A"Machine_Cut: Move to Cut Pin Location"
G1 X{var.pin_park_x_loc} Y{var.pin_park_y_loc} F{global.Machine_travel_speed[0] * 60}                                                    ; Move to the calculated park position near the cut pin.

M98 P"0:/sys/AFC/Macros/debug.g" A"AFC_Cut: Cut Move..."

; --- Increase Motor Current for Cutting Force (M906) ---
if global.Machine_cut_current_stepper[0] > 0
    M906 X{global.Machine_cut_current_stepper[0]}                                                                                      ; Temporarily increase X motor current.
if global.Machine_cut_current_stepper[1] > 0
    M906 Y{global.Machine_cut_current_stepper[1]}                                                                                      ; Temporarily increase Y motor current.
if global.Machine_cut_current_stepper[2] > 0
    M906 Z{global.Machine_cut_current_stepper[2]}                                                                                      ; Temporarily increase Z motor current.

; --- Cutting Distance Calculation ---
set var.cut_dist = global.Machine_cut_dist[0] + global.Machine_cut_dist[1]                                                                   ; Calculate total cut travel distance.

; --- Multi-Pass Cutting Loop (N-1 passes) ---
while iterations < {global.Machine_cut_move[6] - 1}
    ...
    if global.Machine_cut_direction == "back"
        set var.location_factor = {0, 1}                                                                                            ; Actual cut move direction (Y+).
                                                                                                                                   ...
                                                                                                                                   ; --- X-Axis Cut Pass ---
    if global.Machine_cut_direction == "left" || global.Machine_cut_direction == "right"
        set var.fast_slow_transition_loc_x = var.pin_park_x_loc + var.location_factor[0] * (var.cut_dist * global.Machine_cut_move[4]) ; Calculate fast-to-slow transition X point.
        ...
        if var.full_cut_loc_x > var.xmax || var.full_cut_loc_x < var.xmin                                                          ; Safety Check: Check if final X position is within limits.
            ...
            G1 X{var.full_cut_loc_x} F{global.Machine_cut_move[1] * 60}                                                                  ; Move to final cut point (Slow, High-Force Cut).
            G4 P{global.Machine_cut_move[3]}                                                                                           ; Dwell after cut.
            G4 P200
            G1 X{var.pin_park_x_loc} F{global.Machine_cut_move[2] * 60}                                                                  ; Move back to the initial park position (Fast Return).

                                                                                                                                   ; --- Y-Axis Cut Pass ---
    elif global.Machine_cut_direction == "front" || global.Machine_cut_direction == "back"
        set var.fast_slow_transition_loc_y = var.pin_park_y_loc + var.location_factor[1] * (var.cut_dist * global.Machine_cut_move[4]) ; Calculate transition Y point.
        ...
        if var.full_cut_loc_y > var.ymax || var.full_cut_loc_y < var.ymin                                                          ; Safety Check: Check if final Y position is within limits.
            ...
            G1 Y{var.full_cut_loc_y} F{global.Machine_cut_move[1] * 60}                                                                  ; Slow, high-force cut move.
            G4 P{global.Machine_cut_move[3]}                                                                                           ; Dwell after cut.
            G4 P200
...
; --- Final Pass and Rip ---
M98 P"0:/sys/AFC/Macros/debug.g" A"Machine_Cut: Final Move..."
...
if global.Machine_cut_direction == "left" || global.Machine_cut_direction == "right"
    set var.fast_slow_transition_loc_x = var.pin_park_x_loc + var.location_factor[0] * (var.cut_dist * global.Machine_cut_move[4])
    ...
    if var.full_cut_loc_x > var.xmax || var.full_cut_loc_x < var.xmin
        ...
        G1 X{var.full_cut_loc_x} F{global.Machine_cut_move[1] * 60}                                                                      ; Final slow cut move.
        G4 P{global.Machine_cut_move[3]}                                                                                               ; Dwell.
        if global.Machine_cut_rip[0] > 0                                                                                               ; Check if rip/pullback is configured.
            G1 E{-global.Machine_cut_rip[0]} F{global.Machine_cut_rip[1] * 60}                                                             ; Perform E-retract to tear/rip filament free.
        G4 P200
        G1 X{var.pin_park_x_loc} F{global.Machine_cut_move[2] * 60}                                                                      ; Return to park position.

; --- Y-Axis Final Cut and Rip Routine ---
elif global.Machine_cut_direction == "front" || global.Machine_cut_direction == "back"
    set var.fast_slow_transition_loc_y = var.pin_park_y_loc + var.location_factor[1] * (var.cut_dist * global.Machine_cut_move[4])
    ...
    if var.full_cut_loc_y > var.ymax || var.full_cut_loc_y < var.ymin
        ...
        G1 Y{var.full_cut_loc_y} F{global.Machine_cut_move[1] * 60}                                                                      ; Final slow cut move.
        G4 P{global.Machine_cut_move[3]}
        if global.Machine_cut_rip[0] > 0
            G1 E{-global.Machine_cut_rip[0]} F{global.Machine_cut_rip[1] * 60}                                                             ; Perform E-retract to tear/rip filament free.
        G4 P200
...
set var.extruder_move_dist = var.extruder_move_dist + global.Machine_cut_rip[0]                                                        ; Update E distance tracked.

; --- Tip Pushback (Optional Cleanup) ---
if global.Machine_cut_pushback[0] > 0
    M98 P"0:/sys/AFC/Macros/debug.g" A"Machine_Cut: Push cut tip back into hotend"
    G1 E{global.Machine_cut_pushback[0]} F{global.Machine_cut_move[5] * 60}                                                                  ; Push E forward slightly to clear entrance.
    G4 P{global.Machine_cut_pushback[1]}                                                                                               ; Dwell.
    G1 E{-global.Machine_cut_pushback[0]} F{global.Machine_cut_move[5] * 60}                                                                 ; Retract E back.
    set var.extruder_move_dist = var.extruder_move_dist + global.Machine_cut_pushback[0]                                               ; Update E distance tracked.
