...
var max_z = move.axes[global.Machine_axis_number[2]].max                  ; Capture Z axis maximum machine limit. (Not used below, but useful for context/debugging)
var cur_z = move.axes[global.Machine_axis_number[2]].machinePosition      ; Capture current Z position.
...
; Ensures the coordinate system is valid before attempting absolute moves.
                                                                          ; Check if X (axis 0), Y (axis 1), or Z (axis 2) are not homed.
...
set var.z_safe = max(global.Machine_park[2], move.axes[global.Machine_axis_number[2]].machinePosition)
...
G1 X{global.Machine_park[0]} Y{global.Machine_park[1]} F{var.travel_speed_xy} ; Move X/Y to the configured park coordinates (mm/min speed).