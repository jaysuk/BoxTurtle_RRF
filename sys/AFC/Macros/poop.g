...
var purge_speed_mm_min = global.Machine_purge_speed * 60

M98 P"0:/sys/AFC/Macros/debug.g" A"AFC_Poop: Starting poop"                                             ; Log the start of the purge routine.

; --- Part Cooling Fan Setup ---
if global.Machine_part_cooling_fan[0]                                                                   ; Check if fan control is enabled (index 0 is likely a boolean flag).
    M98 P"0:/sys/AFC/Macros/debug.g" A"AFC_Poop: Set Cooling Fan to Full Speed"
    set var.backup_fan_speed = fans[tools[0].fans[0]].requestedValue                                    ; Capture original fan speed.
    M400                                                                                                ; Wait for moves to finish before changing fan state.
    M106 S{global.Machine_part_cooling_fan[1]}                                                          ; Set fan to the configured full speed value (index 1).
...
set var.purge_len = global.Machine_purge_length[0]                                                      ; Set target purge length from configuration (index 0).

; Apply purge minimum
set var.purge_len = max(var.purge_len,global.Machine_purge_length[1])                                   ; Enforce a minimum purge length (index 1).

M98 P"0:/sys/AFC/Macros/debug.g" A"AFC_Poop: Move To Purge Location"                                    ; Log move to purge location.

; --- Move to Start Location ---
G1 X{global.Machine_purge_location[0]} Y{global.Machine_purge_location[1]} F{var.travel_speed_xy}       ; Move XY to the configured purge coordinates (mm/min speed).

if global.Machine_z_purge_move                                                                          ; Check if Z movement is enabled for purging.
    G1 Z{var.purge_z+global.Machine_purge_start} F{var.travel_speed_z}                                  ; Move Z to the starting height (base Z + configured offset).
...
while iterations < var.max_iterations                                                                   ; Loop executes once per segment of max_iteration_length.
    M98 P"0:/sys/AFC/Macros/debug.g" A{"AFC_Poop: Iteration " ^ iterations ^ ", Blob Step " ^ var.step} ; Log current pass number.
    ...
    if var.step == 0                                                                                    ; If this is the start of a new blob cycle (new layer).
        if global.Machine_z_purge_move                                                                  ; If Z movement is enabled.
            G1 Z{var.purge_z+global.Machine_purge_start} F{var.travel_speed_z}                          ; Reset Z to the base starting height.
    ...
    if var.step == 0
        set var.z_raise_substract = global.Machine_purge_start                                          ; If first step, subtract the starting Z offset.
    else 
    ...
    if global.Machine_z_purge_move                                                                      ; If Z movement is enabled.
        ...
        G1 E{var.extrude_amount} F{var.purge_speed_mm_min}                                              ; Pure extrusion move (no Z lift).
    ...
    if var.max_iterations_reached || var.purge_length_reached                                           ; If cycle ends or total length is met.
        ...
        G4 S{global.Machine_part_cooling_fan[2]}                                                        ; Dwell/Wait for pressure to release (index 2 is likely the time in seconds).

G90                                                                                                     ; Switch back to Absolute Positioning mode.

; --- Final Z Lift and Restoration ---
M98 P"0:/sys/AFC/Macros/debug.g" A"AFC_Poop: Fast Z Lift to keep poop from sticking"                    ; Log fast lift.

if global.Machine_z_purge_move
    G1 Z{global.Machine_purge_fast_z[1]} F{global.Machine_purge_fast_z[0] * 60}                         ; Perform a fast Z move to the final park height (index 1) to break the purged blob free.

M98 P"0:/sys/AFC/Macros/debug.g" A"AFC_Poop: Restore fan speed and feedrate"                            ; Log restoration.

if global.Machine_part_cooling_fan[0]                                                                   ; Check if fan was controlled.
