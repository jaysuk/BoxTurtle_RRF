...
; This does not come from the klipper implementation
set global.AFC_stepper_direction = {0,0,0,0,0,0,0,0}
...
; This does not come from the klipper implementation
set global.AFC_microsteps = {16,16,16,16,16,16,16,16}

; This sets the steps per mm of each lane. There should be no need to adjust this with a default AFC build
; This does not come from the klipper implementation
set global.AFC_steps_per_mm = {682,682,682,682,682,682,682,682}

; This is the current used by each stepper driver. 
; This is in mA.
; There should be no need to adjust this with a default AFC build
; This does not come from the klipper implementation
set global.AFC_stepper_current = {1000,1000,1000,1000,1000,1000,1000,1000}

; This is the jerk setting for motor jerk set per lane. 
; These are in mm/s
; There should be no need to adjust this with a default AFC build
; This does not come from the klipper implementation
set global.AFC_stepper_jerk = {60,60,60,60,60,60,60,60}                             ; lanes 0 to 3 and then for the extruder

; This is the maximum speed for each lane. 
; These are in mm/s
; There should be no need to adjust this with a default AFC build
; This does not come from the klipper implementation
set global.AFC_stepper_max_speed = {60,60,60,60,60,60,60,60}                   ; lanes 0 to 3 and then for the extruder

; This is the maximum acceleration for each lane. 
; These are in mm/s2
; There should be no need to adjust this with a default AFC build
; This does not come from the klipper implementation
set global.AFC_stepper_acc = {3000,3000,3000,3000,3000,3000,3000,3000}
...
set global.AFC_max_load_length = 20000
...
set global.AFC_trigger_numbers = {2,3,4,5,6,7,8,9}
set global.AFC_buffer_trigger_numbers = {10,11}

; These are the used with the triggers to create the inputs
; The gcode these are used with is the J value in M950
; These will need adjusting if you have any other triggers setup.
set global.AFC_trigger_input_numbers = {0,1,2,3,4,5,6,7}
set global.AFC_buffer_input_numbers = {8,9}
set global.AFC_hub_input_number = 10
set global.AFC_unload_input_number = 11
...
set global.AFC_hub_load_distance = {35, 5}
...
; This is to set a leading character to the pin
; ! = inverted
; ^ = pullup
var l_s_p = "^"
...
set global.extruder_switches = { "nil", "nil" }
...
set global.AFC_neopixel_settings = {0,2,global.AFC_total_lanes}
...
set global.AFC_z_purge_move = true 
...
set global.AFC_brush = {-1,-1,-1,150,30,10,true,4,false,"0.PE6",110,20,10,0,false}
...
set global.AFC_driver_number = vector(global.AFC_total_lanes, 0)
...
set global.AFC_SLP_pins = vector(global.AFC_total_lanes, "")
while iterations < global.AFC_total_lanes
    set global.AFC_SLP_pins[iterations] = global.AFC_CAN_address^"."^var.SLP_pins[iterations]

set global.AFC_DC1_pins = vector(global.AFC_total_lanes, "")
while iterations < global.AFC_total_lanes
    set global.AFC_DC1_pins[iterations] = global.AFC_CAN_address^"."^var.DC1_pins[iterations]

set global.AFC_DC2_pins = vector(global.AFC_total_lanes, "")
while iterations < global.AFC_total_lanes
    set global.AFC_DC2_pins[iterations] = global.AFC_CAN_address^"."^var.DC2_pins[iterations]

set global.AFC_prep_switch = vector(global.AFC_total_lanes, "")
while iterations < global.AFC_total_lanes
    set global.AFC_prep_switch[iterations] = var.p_s_p^global.AFC_CAN_address^"."^var.p_s[iterations]

set global.AFC_load_switch = vector(global.AFC_total_lanes, "")
while iterations < global.AFC_total_lanes
    set global.AFC_load_switch[iterations] = var.l_s_p^global.AFC_CAN_address^"."^var.l_s[iterations]

set global.TN_switches = {{var.tn_s_p^global.AFC_CAN_address^"."^var.turtleneck_switches[0]},{var.tn_s_p^global.AFC_CAN_address^"."^var.turtleneck_switches[1]}}                                ; These are in the order of Advance and Trailing
set global.AFC_hub_switch = {var.h_s_p^global.AFC_CAN_address^"."^var.hub_switch}
set global.AFC_neopixel_pin = {global.AFC_CAN_address^"."^var.neopixel_pin}

set global.AFC_tmp_file = "0:/sys/AFC/AFC-info/tmp.g"

set global.AFC_features={var.brush,var.cut,var.kick,var.park,var.poop,var.purge,var.load,var.start,var.dc,var.unload,var.spoolman}

; ########## Button Lists ##########

set global.M291_lane_list = vector(global.AFC_total_lanes, "") 
while iterations < global.AFC_total_lanes
    set global.M291_lane_list[iterations] = "Lane " ^ iterations

set global.M291_lane_list_all = vector(global.AFC_total_lanes+1, "") 
while iterations < global.AFC_total_lanes
